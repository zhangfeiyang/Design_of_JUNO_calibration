...
The other effect is about energy non-linearity of detector response. The quenching of LS is different for different energy, and even with the same energy, e-, e+ and gamma still have different quenching, so the quenching of LS is energy and particle dependent. Reference to Ref.~\cite{lab1}, the incorrect energy scale will give wrong MH sensitivity (increase or decrease). So control of energy scale uncertainty is essential.

\subsection{Calibration system}

\begin{center}
	\includegraphics[width=3.0in]{calibration_system.png}
	\figcaption{The overview of calibration system, including automatic calibration Unit (ACU), cable loop system (CLS), Guide Tube (GT) and Remotely Operated Under-liquid-scintillator Vehicles (ROV)}
	\label{overview_calibration_system}
\end{center}
During the calibration, we need deployment system to move the source to various position. 
...
\subsubsection{Automatic Calibration Unit (ACU)}
...
ROV is 3D scanning system, meaning that we can move the calibration source to nearly everywhere in the CD. ROV is not a routine calibration system, but it still essential as a supplement of ACU, CLS, and GT.
...
In this study, we mainly used JUNO official software SNIPER to do simulation. The SNIPER is developed based on Geant4. The framework includes particles generation, detector simulation, calibration and reconstruction, The version of SNIPER used in this study is J17-v1-r1-pre1 released by JUNO software group.

\subsection{Introduction of source geometry}
The geometries of radioactive source have been written into SNIPER with gdml files. 
There are three parts for every radioactive source,including source enclosure, weight and quick connector. The total distance from top to bottom is 50 cm, and two cables are used to combine them together. The length of every cable is about 20 cm, which can make sure small shadowing effect from weight and quick connector. 


The internal part is stainless steel to seal the source. Both diameter and height of the SS are 6mm, which is very small to decrease the energy loss, with two loops at top and bottom to attach the cable. The outer is the PTFE shell to increase the reflectivity to minimize the photon loss. 
...
The neutron enclosure is different from gamma since smaller neutron source is not safe and necessary. Both diameter and height of neutron source are 8 mm. The source is in a stainless steel (SS) tube. There are two SS pins respectively in top and bottom of the tube. The outer is the PTFE shell just same as gamma source.
...
The radioactive sources include routine gamma and neutron sources.
The gamma sources have a small size of geometry, with low energy loss and optical shadowing. So we could have a high precise calibration of gamma energy.
However, only gamma source is not enough for the high energy calibration. So neutron source is necessary, which can provide 2.22 MeV gamma with neutron captured by hydragen and 4.95 MeV captured by carbon and 6.13 MeV with the second exited stat.
The detail of calibration sources and correlated energy is shown in table~\ref{radioactive_source}
...
%The geometry of sources has already been written into SNIPER as shown in figure for both gamma and neutron source. 
The Fig.~\ref{Cs137} shows the simulation of gamma source Cs-137 with source enclosure. The Compton tail due to energy loss in source geometry will shift the full absorption peak. The fit function will correct the shift. 
%The Fig.~\ref{neutron} shows the simulation of neutron source Am-C with source enclosure. Even with source enclosure, there is still no Compton shoulder for neutron source.

\begin{center}
	\includegraphics[width=3in]{Cs137.pdf}
	\figcaption{PE spectrum of Cs137 with source enclosure at CD center.Fit the spectrum with physically-motivated EM calorimeter function in Ref.~\cite{lab4} to obtain full absorption peak. The function can correct the shift of peak by Compton tail.}
	\label{Cs137}
\end{center}
...
\begin{center}
	...
	\figcaption{Optical shadowing effect. The shadowing effect is mainly due to source enclosure and weights. The bias due to shadowing is less than 0.15\% for every source. And the shadowing is obvious energy dependent, which makes sense.}
	\label{shadowing}
\end{center}
...
\begin{center}
	...
	\figcaption{Compton effect. The Compton effect is also energy dependent even it's not very obvious. Since Co60 has two gammas with average 1.25 MeV energy, it have higher probability of Compton scattring in non-LS compared with single 1.25 MeV gamma. }
	\label{compton}
\end{center}

\section{Control energy uncertainty}
\subsection{Shadowing effect}
The material of source container shell is Teflon. Even the reflectivity is very high (assume 90\% reflectivity in the simulation), there still must be some optical photon loss. So we need to study the shadowing effect. To decouple the shadowing effect and Compton effect, we can select the events without energy loss during the simulation. With the cut, the PE spectrum follows the Gaussian distribution, so we fit it with Gaussian function to obtain mean value. And then compare it with the ideal case, bare source without enclosure. Fig.~\ref{shadowing} shows the uncertainty due to the shadowing effect.

\subsection{Compton effect}
The Compton effect is due to energy loss in non-LS material, and this will introduce Compton shoulder in PE spectrum. And the Compton shoulder will shift the mean value. Even we use physically-motivated EM calorimeter function in Ref.~\cite{lab4} to correct this effect, there is still some uncertainty we should consider. Fig.~\ref{compton} the uncertain due to the Compton effect.

\subsection{Electronic effect}
The non-linearity of electronic can be calibrated with our lased calibration system. The intensity of laser calibration system can be controled to 0.3\% uncertainty in Ref.~\cite{lab2}, which can give a good calibration for electronic electronic. So in this paper, we roughly assume the uncertainty from electronic non-linearity is 0.3\%. 
...
Since the requirement of uncertainty is 0.3\%, it's better to control the statistical uncertainty less than 0.03\%.
Based on this, we did the simulation for all the source. We need about 30000 events for every source.
For every source calibration, it will take ~5 minutes. So the rate of every source should be about 100 Hz.

\begin{center}
	\centering
	\includegraphics[width=3in]{stat_uncertainty.pdf}
	\figcaption{Statistic uncertainty. The uncertainty is significantly energy dependent, which make sennse.}
	\label{statistical}
\end{center}

\section{Energy scale}
One of main purposes of JUNO calibration is to control uncertainty of the scale less than 1\%. In order to reach this physics requirement, we should have a better understanding of the energy non-linearity of detector response. A series of radioactive sources will be used to do the calibration of energy non-linearity, including gamma sources, neutron sources and beta source. 

\subsection{Energy non-linearity of gamma}
Here we will introduce the strategy of JUNO calibration based on MC simulation result. The full absorption peak of delay signal of neutron capture at Hydrogen (2.22 MeV) is determined as the energy scale. Then we can reconstruct energy of other calibration source with PE/scale. The reconstructed energy is different from true energy due to energy non-linearity. The ratio of reconstructed energy and true energy will be described as energy dependent of non-linearity.
...
However, IBD events is positron events. So we would like to know the response of positron energy non-linearity. 
The relation between gamma and electron can be established with Geant4. 
And we assume that electron and positron have almost same action of energy non-linearity in addition to two gammas from positron annihilation.
There are mainly three physics process for the conversion from gamma to electron, including pair production, Compton scattering, photoelectric effect. 
Based on the MC simulation, we can obtain the probability density function (PDF) of electron energy with mono energy gamma.
  

\subsection{Model of energy non-linearity}
Referring to Daya Bay's method in Ref.~\cite{lab3}, an empirical formula with 4 parameters was used to describe energy non-linearity of electron, 
\begin{eqnarray}
\label{eq1}
...
The top figure show the non-linearity of gamma with ratio of True energy and reconstruted one.
The black point is data from calibration of gamma, while the red line shows the best fit with Eq.~(\ref{eq1}).
The bottom is the ratio of Data and best fit.
The error bar is the combination of uncertainties including optical shadowing, Compton effect, electronic non-linearity and statistic.}
...
\begin{center}
	...
	\figcaption{Bias of positron non-linearity. The non-linearity of positron is obtained by fitting data of gamma non-linearity. And then comparing the reconstruted non-linearity and true non-linearity in MC simulation, we get the bias of two.}
	\label{bias_positron}
\end{center}
...
\begin{center}
	...
	\figcaption{Systematic uncertainty analysis. The ratio of reconstructed energy and true one shows the bias of non-linearity after calibration for positron. The error bar show the uncertainty. The overall uncertainty of non-linearity is less than 1\% from 1 to 8 MeV, which meets the physics requirement of MH determination. }
...
The true energy scale could be obtained with ideal source in the MC simulation.
Then the uncertinty can be randomly added for every source based on the study for every type of effect.
Based on the biased energy scale, the non-linearity of positron will be reconstructed. And repeat this procedure multiple times, then we can get the uncertainty of non-linearity compared with the true one as shown in Fig.~\ref{sys_uncertainty_non-linearity}. 
\section{Energy resolution}
The task of calibation is not only for non-linearity but also non-uniformity. The non-uniformity of detector should be corrected with calibration system to contol the energy resolution better than 3\%. 

\begin{center}
	\centering
	\includegraphics[width=3in]{calibration_mapping.pdf}
	\figcaption{Calibration mapping. In order to construct to a better strategy of calibration, first we simuate enough calibration points in the half vertical plane. The points is randomly distribute in the plane. Then we will do optimization based on this mapping.}
...
\begin{center}
	...
	\figcaption{Spline function to describe the non-uniformity.}
...
\begin{center}
	...
	\figcaption{Non uniformity correction. 0 MeV e+ uniformly distribute in CD. The spline function is used to correction the non-uniformity based on the true vertex for every event.}
	\label{mono_energy}
\end{center}
...
\begin{center}
	...
	\figcaption{Energy resolution as function of two anchors positions. The best choices are respectively 48 degree and 78 degree based on energy resolution.}
	\label{best_anchor}
\end{center}
...
\subsection{Systematic uncertainty analysis.}
Vertex reconstruction. The non-uniformity correction depends on vertex of events, so vertex reconstruction significantly affects the quality of non-uniformity correction. 
Here we use the assumption of vertex reconstruction in Ref.~\cite{lab1}, 8 cm @ 1MeV. As shown in table~\ref{summary_resolution}, with vertex smearing, the overall energy resolution is 0.02\% worse than before.
We also assume different vertex reconstruction 10 cm/$\sqrt{E(MeV)}$ and 15 cm/$\sqrt{E(MeV)}$, and get corresponding energy resolution 2.94\% and 2.96\%.
...
Table~\ref{summary_resolution} is a summary of energy resolution with different facts. The PMT dark noise is not inclued in this simuation, but we can roughly estimate the value in Ref~\cite{lab1}.


\section{Conceptual calibration strategy}
The conceptual calibration strategy is based on above simulation result. There two types for the routine calibration, weekly calibration and monthly calibration.
...
The weekly calibration can monitor the variation of energy scale.
During the weekly calibration, only ACU will be executed.


\subsection{Monthly calibration}
The monthly calibration will be operated every month. During the monthly calibration, the ACU, CLS, and GT will be operated in table~\ref{monthly_time}. 
The monthly calibration can correct the detector non-uniformity.
...
\begin{center}
	...
	\begin{tabular*}{170mm}{@{\extracolsep{\fill}}ccccccc}
		...
		positioning precision & 2.75(1) & 0.70(1) & 0 & 2.97(1)\\
		dark noise & 2.75(1) & 0.70(1) & 0.9& 3.02(1)\\
		\bottomrule  % ???
...
\begin{center}
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular*}{170mm}{@{\extracolsep{\fill}}cccccccc}
			...
			\midrule  % ???
			ACU&Neutron (Am-C) &   21 & 133 & 105 & 238\\
			CLS&Neutron (Am-C) &   219 & 340 & 1095 & 1435\\
			GT&Neutron (Am-C)  &   11 & 40 & 55 & 95\\
			Total&/&/&/&/&1768\\
			\bottomrule  % ???
	\end{tabular*}}
	\label{monthly_time}
...
\bibitem{lab2}Y. Zhang et al. 2019 JINST 14 P01009
\bibitem{lab3}F. P. An et al. 2014 Phys. Rev. Lett. 112, 061801
\bibitem{lab4}Jia-Hua Cheng et al. arXiv 1603.04433
